-- Fix visitor analytics to properly count all visitors and locations
...
BEGIN
  ...
  -- Create a session-level summary with location data
  ...
  analytics AS (
    SELECT
      ...
      COUNT(DISTINCT COALESCE(s.ip_address::text, s.session_id)) AS unique_visitors,
      COUNT(DISTINCT v.page_path) AS unique_pages,
      COUNT(CASE WHEN v.visit_timestamp > NOW() - INTERVAL '24 hours' THEN 1 END) AS views_last_24h,
      COUNT(DISTINCT CASE WHEN v.visit_timestamp > NOW() - INTERVAL '24 hours' THEN v.session_id END) AS sessions_last_24h
    FROM views_only v
    LEFT JOIN session_summary s ON s.session_id = v.session_id
  ),
  top_pages AS (
    SELECT
      jsonb_agg(
  ...
  location_data AS (
    SELECT
      ...
      ) FILTER (WHERE rn <= 20) AS locations
    FROM (
      SELECT
        country,
        region,
    ...
    SELECT
      jsonb_agg(
        jsonb_build_object(
          ...
          'country', COALESCE(ss.country, 'Unknown'),
          'region', ss.region,

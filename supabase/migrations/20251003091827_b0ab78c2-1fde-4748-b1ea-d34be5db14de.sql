-- Align unique_visitors to unique sessions and ensure consistent aggregations
...
BEGIN
  ...
  avg_time_calc AS (
    SELECT COALESCE(ROUND(AVG(time_on_page))::int, 0) AS avg_time
    FROM time_events
  ),
  analytics AS (
    SELECT
      ...
      COUNT(DISTINCT v.session_id) AS unique_visitors, -- align with UI expectation

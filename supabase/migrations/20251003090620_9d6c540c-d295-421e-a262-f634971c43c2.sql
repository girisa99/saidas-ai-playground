-- Fix inconsistent analytics by counting only entry "view" events for views and deriving time from dedicated time events
-- and by handling NULL IPs via session fallback. This replaces the existing function.
...
BEGIN
  ...
  analytics AS (
    SELECT
      ...
      ROUND(
        COALESCE(
          (SELECT AVG(te.time_on_page) FROM time_events te),
          0
        )
      )::int AS avg_time_on_page,
  ...
  session_journeys AS (
    SELECT
      jsonb_agg(
        jsonb_build_object(
          ...
          'total_time_seconds', COALESCE(s.total_time_seconds, 0),
    ...
    FROM (
      SELECT
        ...
        COALESCE((SELECT SUM(te.time_on_page) FROM time_events te WHERE te.session_id = v.session_id), 0)::int AS total_time_seconds,
